...
function Main ({ children, onMiniMenu, openMiniMenu, close }) {
  return (
    <main className={`relative flex h-full min-w-full flex-auto items-start gap-6 bg-kcianli px-5 py-6 transition duration-500 dark:bg-kblackli ${MiniMenu ? 'overflow-x-hidden' : 'overflow-x-scroll'}`}>
      {children }
      {openMiniMenu &&
